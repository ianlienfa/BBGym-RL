...
{        
    ...
    std::tuple<float, float, vector<float>> train(vector<float> flatten, int operator_option);
    float label_decision(std::tuple<float, float, vector<float>>, bool exploration=false);
